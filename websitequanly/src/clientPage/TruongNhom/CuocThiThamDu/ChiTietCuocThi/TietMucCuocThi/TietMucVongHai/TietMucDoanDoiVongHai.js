...
export default function TietMucDoanDoiVongHai(props) {
    ...
    const getDataCuocThiTuDoCaNhan = async () => {
      const { data } = await Axios.post(
        `http://localhost:3001/api/client/cuocthi/${
          params.idCuocThi
        }/vongthi/2/truongnhom/${localStorage.getItem("MaNguoiDung")}`
      );
      data.forEach((item) => {
        if (item.CongBoKetQua == 0) {
          ...
          if (item.SoVongThi == item.VongThi) {
            item.TrangThai = item.TenGiaiThuong;
          } else {
