...
export default function TietMucVongHai(props) {
      ...
      if (props.phanThi == 1) {
        Axios.post(
          `http://localhost:3001/api/client/tietmucchamdiemcuocthicanhan/${
            params.idCuocThi
          }/vongthi/2/giamkhao/${localStorage.getItem("MaNguoiDung")}/ngay`,
          {
            NgayBatDau: batDau,
            NgayKetThuc: ketThuc,
          }
  ...
  const columnsTietMuc = [
    ...
    },
    {
      ...
      options: {
        ...
        customBodyRender: (value, tableMeta, updateValue) => {
          return (
            <div style={{ maxWidth: "60px", marginLeft: "25px" }}>
              {value == null ? "..." : value}
            </div>
          );
        },
    ...
    if (props.phanThi == 1) {
      const getDataCuocThiTuDoCaNhan = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/client/tietmucchamdiemcuocthicanhan/${
            params.idCuocThi
          }/vongthi/2/giamkhao/${localStorage.getItem("MaNguoiDung")}`
        );
        let arr = [];
        data.forEach((d) => {
