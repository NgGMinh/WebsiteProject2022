...
export default function TietMucVongBa(props) {
      ...
      if (props.phanThi == 1) {
        Axios.post(
          `http://localhost:3001/api/client/tietmucchamdiemcuocthicanhan/${
            params.idCuocThi
          }/vongthi/3/giamkhao/${localStorage.getItem("MaNguoiDung")}/ngay`,
          {
            NgayBatDau: batDau,
            NgayKetThuc: ketThuc,
          }
    ...
    if (props.phanThi == 1) {
      const getDataCuocThiTuDoCaNhan = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/client/tietmucchamdiemcuocthicanhan/${
            params.idCuocThi
          }/vongthi/3/giamkhao/${localStorage.getItem("MaNguoiDung")}`
        );
        let arr = [];
        data.forEach((d) => {
