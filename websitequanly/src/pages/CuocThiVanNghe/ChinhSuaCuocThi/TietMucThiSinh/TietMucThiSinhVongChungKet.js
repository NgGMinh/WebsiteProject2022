...
export default function TietMucThiSinhVongChungKet(props) {
  ...
  const handleFilterCN = () => {
    if (batDau == null) alert("Chưa chọn Ngày Bắt đầu!");
      ...
      Axios.post(
        `http://localhost:3001/api/admin/tietmucthuoccuocthicanhan/${params.idCuocThi}/vongthi/3/ngay`,
        {
          NgayBatDau: batDau,
          NgayKetThuc: ketThuc,
        }
  ...
  const handleFilterDN = () => {
    if (batDau == null) alert("Chưa chọn Ngày Bắt đầu!");
      ...
      Axios.post(
        `http://localhost:3001/api/admin/tietmucthuoccuocthidoinhom/${params.idCuocThi}/vongthi/3/ngay`,
        {
          NgayBatDau: batDau,
          NgayKetThuc: ketThuc,
        }
  ...
  const handleChangeCongBo = (cb) => {
    Axios.post(
      `http://localhost:3001/api/admin/updatecongbo/tudo/${params.idCuocThi}/3`,
      {
        CongBoKetQua: cb,
      }
    ).then((response) => {
            ...
            <MuiToolTip title={"Thêm Tiết Mục"}>
              <IconButton
                ...
                onClick={() => {
                  if (props.phanThi == 1) {
                    navigate(
                      `/themtietmuccuocthi/canhan/${params.idCuocThi}/vongthi/3`
                    );
                  } else {
                    navigate(
                      `/themtietmuccuocthi/doinhom/${params.idCuocThi}/vongthi/3`
                    );
                  }
                }}
              >
    ...
    if (props.phanThi == 1) {
      const getDataCuocThiTuDoCaNhan = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/admin/tietmucthuoccuocthicanhan/${params.idCuocThi}/vongthi/3`
        );
        data.forEach((d) => {
          if (d.NgayGioThucHien == null) {
            d.NgayGioThucHien = "Chưa sắp lịch";
      ...
      const getDataCuocThiTuDoDoiNhom = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/admin/tietmucthuoccuocthidoinhom/${params.idCuocThi}/vongthi/3`
        );
        data.forEach((d) => {
          if (d.NgayGioThucHien == null) {
            d.NgayGioThucHien = "Chưa sắp lịch";
  ...
  useEffect(() => {
    if (props.phanThi == 1) {
      const getDataThiSinhCuocThiCaNhan = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/admin/thisinhcuocthicanhan/${params.idCuocThi}/vongthi/3`
        );
        data.forEach((d) => {
    ...
    if (props.phanThi == 2) {
      const getDataThiSinhCuocThiDoiNhom = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/admin/thisinhcuocthidoinhom/${params.idCuocThi}/vongthi/3`
        );
        data.forEach((d) => {
          d.GioiTinh = d.GioiTinh == 1 ? "Nam" : "Nữ";
          if (d.MaDinhDanh == null || d.MaDinhDanh == "") {
