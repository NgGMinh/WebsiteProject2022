...
import { IconButton, Tooltip as MuiToolTip, Button } from "@mui/material";
...
export default function TietMucThiSinhVongSoTuyen(props) {
  ...
  const handleFilterCN = () => {
    if (batDau == null) alert("Chưa chọn Ngày Bắt đầu!");
      ...
      Axios.post(
        `http://localhost:3001/api/admin/tietmucthuoccuocthicanhan/${params.idCuocThi}/vongthi/1/ngay`,
        {
          NgayBatDau: batDau,
          NgayKetThuc: ketThuc,
        }
  ...
  const handleFilterDN = () => {
    if (batDau == null) alert("Chưa chọn Ngày Bắt đầu!");
      ...
      Axios.post(
        `http://localhost:3001/api/admin/tietmucthuoccuocthidoinhom/${params.idCuocThi}/vongthi/1/ngay`,
        {
          NgayBatDau: batDau,
          NgayKetThuc: ketThuc,
        }
  ...
  const handleChangeCongBo = (cb) => {
    Axios.post(
      `http://localhost:3001/api/admin/updatecongbo/tudo/${params.idCuocThi}/1`,
      {
        CongBoKetQua: cb,
      }
    ).then((response) => {
  ...
  const handleChangeKetQua = (idTM, kq) => {
    Axios.post(
      `http://localhost:3001/api/admin/updatetrangthaitietmuc/${params.idCuocThi}/1`,
      {
        MaTietMuc: idTM,
        TrangThai: kq,
      }
            ...
            <MuiToolTip title={"Thêm Tiết Mục"}>
              <IconButton
                ...
                onClick={() => {
                  if (props.phanThi == 1) {
                    navigate(
                      `/themtietmuccuocthi/canhan/${params.idCuocThi}/vongthi/1`
                    );
                  } else {
                    navigate(
                      `/themtietmuccuocthi/doinhom/${params.idCuocThi}/vongthi/1`
                    );
                  }
                }}
              >
    ...
    if (props.phanThi == 1) {
      const getDataCuocThiTuDoCaNhan = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/admin/tietmucthuoccuocthicanhan/${params.idCuocThi}/vongthi/1`
        );
        data.forEach((d) => {
          if (d.NgayGioThucHien == null) {
            d.NgayGioThucHien = "Chưa sắp lịch";
      ...
      const getDataCuocThiTuDoDoiNhom = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/admin/tietmucthuoccuocthidoinhom/${params.idCuocThi}/vongthi/1`
        );
        data.forEach((d) => {
          if (d.NgayGioThucHien == null) {
            d.NgayGioThucHien = "Chưa sắp lịch";
  ...
  useEffect(() => {
    if (props.phanThi == 1) {
      const getDataThiSinhCuocThiCaNhan = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/admin/thisinhcuocthicanhan/${params.idCuocThi}/vongthi/1`
        );
        data.forEach((d) => {
    ...
    if (props.phanThi == 2) {
      const getDataThiSinhCuocThiDoiNhom = async () => {
        const { data } = await Axios.post(
          `http://localhost:3001/api/admin/thisinhcuocthidoinhom/${params.idCuocThi}/vongthi/1`
        );
        data.forEach((d) => {
          d.GioiTinh = d.GioiTinh == 1 ? "Nam" : "Nữ";
          if (d.MaDinhDanh == null || d.MaDinhDanh == "") {
